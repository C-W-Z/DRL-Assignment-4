...
class Actor(nn.Module):
    ...
    def forward(self, state):
        ...
        action_mean = torch.tanh(action_mean) * (self.upper_bound - self.lower_bound) / 2 + (self.upper_bound + self.lower_bound) / 2
        return action_mean
...
class PPOPolicy(nn.Module):
    def __init__(self, state_dim, action_dim, lower_bound, upper_bound, learning_rate=3e-4,
                 clip_range=0.2, value_coeff=0.5, entropy_coeff=0.01, initial_std=0.5, max_grad_norm=0.5):
        super().__init__()
    ...
    def forward(self, state):
        ...
        std = torch.exp(self.log_std).clamp(1e-6, 10.0)
...
def compute_gae(rewards: np.ndarray, values: np.ndarray, last_value: float, is_last_terminal: bool, gamma: float, gae_lambda: float):
    rewards = np.array(rewards, dtype=np.float32)[:, 0]
    values = np.array(values, dtype=np.float32)[:, 0]
    N = len(rewards)
    ...
    return returns[:, np.newaxis], advantages[:, np.newaxis]
...
def load_checkpoint(policy, buffer, running_stat, checkpoint_path):
    ...
    if policy is not None:
        ...
        policy.optimizer = optim.Adam(policy.parameters(), lr=3e-4)
...
def train_ppo(env_name="humanoid-walk",
              num_episodes_per_update=20,
              max_buffer_steps=20000,
              batch_size=256,
              ...
              initial_std=0.5,
              save_interval=500,
              checkpoint_path="humanoid_walk_checkpoint.pt"):
    ...
    while episode_count < total_episodes:
        ...
        if len(mean_rewards) > 0 and mean_rewards[-1] > 1000:
            print(f"Achieved high reward {mean_rewards[-1]:.2f}. Stopping training.")
            break
...
if __name__ == "__main__":
    checkpoint_path = "humanoid_walk_checkpoint.pt"
    train_ppo(total_episodes=50000, learning_rate=3e-4, checkpoint_path=checkpoint_path)
    train_ppo(total_episodes=75000, learning_rate=1e-4, checkpoint_path=checkpoint_path)
    train_ppo(total_episodes=100000, learning_rate=3e-5, checkpoint_path=checkpoint_path)
